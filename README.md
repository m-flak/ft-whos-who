...
This is my completed Who's Who assessment. Who's Who is a musical guessing game utilizing the Spotify API.

A backend is included to store players' scores in a PostgreSQL database, giving the game a competitive element across multiple computers. **By default,** the web browser's local storage is used instead of the backend.

# Important Notes

For this public demo of Who's Who, the endpoint for acquiring a Spotify API token has been removed.

Assuming you have an endpoint to use, _**src/authEndoint.js**_ should be created with the following contents:
```JavaScript
export default '<INSERT URL HERE>'
```
